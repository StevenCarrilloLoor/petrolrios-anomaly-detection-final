...
/// Página a la que llega el enlace del correo de desbloqueo. Consume el token de un
/// solo uso, levanta el bloqueo de la cuenta y muestra el resultado. El usuario conserva
/// su contraseña: tras esto puede iniciar sesión con normalidad.
export function DesbloquearCuentaPage() {
  ...
  const [resultado, setResultado] = useState<{
    ...
    token
      ? { estado: "cargando", mensaje: "Desbloqueando su cuenta…" }
  ...
  useEffect(() => {
    ...
    authService
      ...
      .then((r) => {
        setResultado({ estado: r.ok ? "ok" : "error", mensaje: r.mensaje });
      })
      .catch(() => {
        setResultado({
          estado: "error",
          mensaje: "El enlace es inválido o expiró. Solicita uno nuevo.",
        });
      });
  }, [token]);
      ...
      <div className="w-full max-w-md rounded-2xl border border-border bg-background p-8 text-center shadow-lg">
        ...
        {resultado.estado === "ok" && (
          <>
            ...
            <h1 className="mt-3 text-xl font-bold">¡Cuenta desbloqueada!</h1>
        ...
        {resultado.estado === "error" && (
          <>
            ...
            <h1 className="mt-3 text-xl font-bold">No se pudo desbloquear</h1>
            <p className="mt-1 text-sm text-muted-foreground">{resultado.mensaje}</p>
            <Link
              to="/login"
