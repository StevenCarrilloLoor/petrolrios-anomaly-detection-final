...
- [ ] 🔴 **Factura no incluida en ninguna liquidación** (cuadre de turno, mejora #3). **Diseño confirmado contra el schema real:** `LIQU` (PK `NUM_LIQU`) tiene `NUM_TURN`, `FEC_LIQU` y totales (`CON_LIQU`/`CRE_LIQU`/`FAL_LIQU`/`SOB_LIQU`/`DIF_LIQU`). Cuadre = factura `DCTO` cuyo `NUM_TURN` **no** aparece en ninguna `LIQU.NUM_TURN` (turno cerrado, no liquidado). **Pasos:** (1) **verificar contra datos reales de San Pío** (ISQL) que `LIQU.NUM_TURN` puebla y que `DCTO.NUM_TURN↔LIQU.NUM_TURN` casa para turnos liquidados — consulta: `SELECT FIRST 20 NUM_LIQU, NUM_TURN, FEC_LIQU FROM LIQU ORDER BY FEC_LIQU DESC;` y cruzar con `DCTO`; (2) **cambiar el agente** (`StationAgent` debe leer `LIQU` y enviarla como tipo "Liquidacion") + **republicar** las 4 plataformas; (3) `LiquidacionDto` + `DetectionContext.Liquidaciones` + `ConstruirContexto`; (4) **regla programada diaria** (solo turnos ya cerrados, p. ej. del día anterior, para no marcar turnos abiertos aún sin liquidar) + tests + seed; (5) gate + Chrome. *Cuidado: no cablear sin verificar el enlace (lección de FAC_DESP).*
