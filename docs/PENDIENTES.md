...
Última actualización: 27 de junio de 2026 (**Ronda ERP/UX EN PROGRESO** — Etapa 1 HECHA `626c3e0` (refresco configurable + agente 1 s), Etapa 2 HECHA `1114d65` (evidencia identificable automática: RUC/nº doc/placa/cliente/turno/fecha/monto en las alertas, 10 reglas + enriquecedor genérico). Faltan: cola de consulta en vivo a Firebird, pestaña Consultas + factura/reportes en ventana nueva, pulido UI, y extender Fuente al resto de reglas. Antes: **Mejora #3 cuadre de turno HECHA** `43fe272` + gate verde + QA E2E en vivo OK: alerta #80 "turno 990001 sin liquidación". Antes, **QA en vivo en Chrome OK** de #1 regla placa, #2 buscador y #4 dashboard por estación; se cazó y arregló en vivo un **500 del buscador** —`MetadataJson` es jsonb, `c011374`—. Lectura completa de las 6 carpetas de `ejecutables`. Antes: #1 `99d3330`, #2 `e5d7bdb`, #4 `594f186`, verificación #3 `cd4008f`).
