...
Última actualización: 27 de junio de 2026 (**Ronda ERP/UX EN PROGRESO** — Etapas 1–4 HECHAS: `626c3e0` (refresco configurable + agente 1 s), `1114d65` (evidencia identificable automática en alertas), `8d0e651`+`7166e40`+`699dbb1`+`5f149a9` (**consulta en vivo a Firebird**: cola on-demand agente+central + pestaña Consultas + factura en ventana nueva). **QA en vivo en Chrome OK** (§92): buscar por RUC largo `1301790737` → **21 facturas FV reales** en la tabla y **factura completa en ventana nueva** (cabecera + importes + Imprimir). Se cazó y arregló en vivo un **truncado de Firebird** (`string right truncation` con RUC de 10+ dígitos): `CAST(col AS VARCHAR(60))` antes de `CONTAINING` + normalización case-insensitive de claves en el frontend. **Falta solo la Etapa 5: pulido UI del detalle de alerta**, + refinamientos (enlazar factura desde la alerta, líneas DESP, extender Fuente al resto de reglas). Antes: **Mejora #3 cuadre de turno HECHA** `43fe272` + gate verde + QA E2E en vivo OK: alerta #80 "turno 990001 sin liquidación". Antes, **QA en vivo en Chrome OK** de #1 regla placa, #2 buscador y #4 dashboard por estación; se cazó y arregló en vivo un **500 del buscador** —`MetadataJson` es jsonb, `c011374`—. Lectura completa de las 6 carpetas de `ejecutables`. Antes: #1 `99d3330`, #2 `e5d7bdb`, #4 `594f186`, verificación #3 `cd4008f`).
