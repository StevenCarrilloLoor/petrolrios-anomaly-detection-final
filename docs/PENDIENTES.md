...
Última actualización: 28 de junio de 2026 (**CONSULTAS: PDF AUTOGENERADO + BÚSQUEDA MULTI-TAG + FACTURAS ENLAZABLES EN LA ALERTA (CAMBIOS §112, `ffb5785`, gate verde 383).** Tres pedidos de la prueba en vivo: (1) **PDF autogenerado** de Consultas con QuestPDF (`POST /api/v1/consultas/pdf`, mismo formato que el reporte de alertas) — botón **PDF** junto a Excel e Imprimir (ya no solo `window.print`); (2) **búsqueda multi-tag**: varios criterios ANDados (placa Y despachador a la vez), botón **+** para añadir y **×** para quitar, escalable (`Codigos[]` en DTO+cola+agente, WHERE dinámico parametrizado; URL `?codigo=` repetido) — **es cambio de AGENTE** (vive para EST-001 al relanzar; SanPio necesita actualizar su agente); (3) en la evidencia de la alerta **cada** documento de `Documentos`/`NumerosFactura` enlaza a su factura (antes solo el suelto) y se quita el `NumeroDocumento` duplicado — genérico, no choca con el creador de reglas. +2 pruebas (PDF) → Api 128. — Antes: **AUDITORÍA DE LAS 25 REGLAS + DESCARGA EXCEL/PDF EN LA FACTURA (CAMBIOS §111, `3555a50`, gate verde 381).** Steven pidió auditar **cada** regla predeterminada verificando que lleve toda la info que un auditor necesita (documento/factura, despachador, placa, RUC, cliente, monto, nombre de combustible). Resultado: 12 reglas de documento único + 6 agregadas + 3 de crédito/tarjeta ya estaban **completas** (heredan evidencia vía `Fuente` o llevan su metadata propio); se enriquecieron las 3 flojas: **AnulacionRecurrente** (lista los comprobantes anulados `est-pto-secuencial` + fechas + tipos + autorizaciones → el auditor jala las facturas), **PrecioFueraLista** (nombre del combustible, no el código) y **DespachoNoFacturado** (nombre por diccionario + `Fuente`=despacho). **Factura:** botón **Excel** (CSV completo: cabecera+cliente+documento+líneas de surtido+importes) y el de imprimir ahora dice **"Imprimir / PDF"**. +2 pruebas → Detectors 205. — Antes: **SISTEMA DE PRECIOS ROBUSTO — POR ETAPAS (CAMBIOS §107).** Steven pidió un sistema de precios totalmente automático/reactivo/robusto (spec de 12 puntos). **Decisión:** scraping robusto y respetuoso (headers de navegador + ETag/304 + backoff + cascada + fallback), **NO** la evasión de anti-bot (rotar identidad/Referer falso de Google) — innecesaria a ~34 req/mes de datos públicos y cruza a evasión. **E1 HECHA (`b9f3541`, gate 353):** Súper referencial; precio **Sistema vs API** + el dashboard muestra ambos (si la API falla, el sistema conserva el suyo); tabla de auditoría `precios_combustible_log`; validación con bandas (rangos + ±10%/±40%); endpoint enriquecido; tarjeta dashboard con badges. **SISTEMA DE PRECIOS COMPLETO E1–E5 (§107–§108), gate verde 379.** E2 cascada de fuentes (`0238142`, parser probado vs el HTML real de gasolinaecuador.com, ETag/304, backoff, fallback al sistema); E3 schedule adaptativo Hangfire (`9e91a20`, normal días 1–10 + alerta 11–12, jitter, idempotente, lógica pura testeable); E5 el **detector USA el precio oficial** (`8ef2715`, tolerancia cero ±1¢ por redondeo del POS, **mapeo confirmado por precio 1=Súper excluida / 2=Extra-Ecopaís $3,31 / 3=Diésel $3,25**, fallback al mínimo del día en histórico); E4 `/salud` + `/historial` + estado escalonado OK/Warning/Error/Critico/Urgente (`bfb2a0f`). **Anti-detección:** construido respetuoso (no evasión de anti-bot). _Para verlo en vivo: reconstruir la API (migración + siembra); el scraping corre solo según el schedule o se fuerza con POST /refrescar._ — Antes: **PRECIOS DE COMBUSTIBLE + FIX AGENTE NUEVO (CAMBIOS §106).** (106.2 `a1b3344`) **API de precios de combustible de Ecuador** (Extra/Ecopaís/Diésel; Súper excluida por no ser regulada): entidad+tabla `precios_combustible` sembrada con los valores oficiales vigentes (12-jun a 11-jul 2026: Extra/Ecopaís $3,31, Diésel $3,25, con subsidios) + `GET/PUT/POST /api/v1/precios-combustible` + conector externo configurable (deshabilitado por defecto, respaldo a lo guardado) + tarjeta `PreciosCombustibleCard` en el dashboard. El admin actualiza la banda cada mes; hay migración `PreciosCombustible` + 4 pruebas. _Para verlo: reconstruir la API (aplica migración + siembra)._ (106.1 `a8e825a`) **Fix agente nuevo:** arrancaba con los defaults del DEMO (NONE + ruta vieja) porque en el primer arranque `appsettings.json` (config del demo) pisaba los buenos defaults; el publicador ahora deja un `config/agent-config.json` de producción (ContaGober/3050/UTF8, identidad en blanco) en cada agente → tildes/Ñ correctas y ruta correcta. _Para verlo: re-publicar el agente._ Gate verde 351. — Antes: **QA EXHAUSTIVA / CAZA DE BUGS (CAMBIOS §104) — barrido por SQL a la BD viva de SanPio; 4 clases de bug halladas y arregladas, gate verde 346.** (104.1 `06b8c89`) **Precio fuera de lista por (producto, DÍA)** no por lote → mata **5 904 falsos positivos** (subida legítima de precio entre días); se limpia solo en el próximo re-sync. (104.2 `06b8c89`) **Monitor incluye Ambos** (filtraba Operativa estricto → mostraba 0 pese a cientos). (104.3 `4aa24d3`) **Reglas programadas (Pasada B) ventanean por LLEGADA `CreatedAt`** no por `FechaOriginal` → ahora ven datos cargados en bloque o que llegan tarde (PlacaReutilizada tenía 79 pares placa/día con +5 facturas y solo 1 alerta). (104.4 `4aa24d3`) **Join factura↔despacho correcto** en MultipleCombustible/PlacaGenerica/AltoVolumenSinPlaca (unían por CodigoManguera "00" de cabecera → nunca casaban; MultipleCombustible daba 0 pese a 274 casos); centralizado en `DetectionRuleBase` por CodigoCliente + cercanía temporal. **No-bugs confirmados:** CashFraud mudo = liquidaciones con Diferencia=0 (SanPio cuadra); acumulación de despachos rápidos impecable (1→38 eventos). **(31) nombres de despachador HECHO (`b1cf5cd`, §105.1):** `EmpleadoDirectorio` resuelve por código exacto y, si falla, por **núcleo numérico** (`DD0000010`→`10`, casa con el catálogo VEND "010"); +1 test. _Operativo:_ el agente de SanPio debe sincronizar su VEND (panel → "Sincronizar ahora"); si VEND no trae esos despachadores, verlo en Explorar tabla → VEND. **(32) rediseño grande del Monitor HECHO (`3ee7d24`, §105.2):** panel local a la altura del central — severidad apilada, filtros por nivel + buscador + orden, evidencia parseada del metadata en cada tarjeta, badge ×N acumulados, detalle expandible (solo-UI, mismos endpoints). Filtro Ambos ya estaba arreglado (§104.2). _Operativo:_ reconstruir/relanzar el Monitor (puerto 5190) para verlo en vivo. **Gate verde 347.** — Antes: **PRODUCCIÓN — ronda G de calibración con datos REALES (estación SanPio EST-015, ~19.8k transacciones del mes). Se recorrió todo en vivo y se documentaron 19 hallazgos (ver `HALLAZGOS-PRODUCCION.md` en outputs). Etapa G3 (encoding) HECHA (`e505405`, CAMBIOS §103): **charset Firebird por defecto = UTF8** → arregla el mojibake de nombres con Ñ/tildes ("MUÑOZ" salía "MUÃ?OZ"; SanPio guarda en UTF-8 y el agente leía con NONE). Configurable por estación. **Para SanPio: FirebirdCharset=UTF8 en el panel + reiniciar.** Nombres de despachador `DD…` sin resolver: **PENDIENTE** (necesita la tabla VEND real de SanPio; diagnóstico: Explorar tabla → VEND). G2 frontend HECHO (`5dbcdd4`): badge "×N acumulados". Etapa G2 HECHA (`d86643f`, CAMBIOS §102): despachos rápidos ahora **ACUMULABLES/ESCALABLES + ámbito Ambos** (idea de Steven) — en vez de 6.080 alertas sueltas, **UNA por caso** que crece y escala (2-3 Medio, 4-5 Alto, 6+ Crítico) re-emergiendo arriba; **dinámica** por RUC/placa, **mínimo 2**; `Alerta` gana `EventosAcumulados`/`FechaActualizacion`/`Acumular`/`EscalarPorConteo` + migración `AlertaAcumulable` + **upsert** en el job; **ruteo Ambos completado** (repo+dashboard+seed). Gate verde 346. CENTRAL (vivo al reconstruir API; aplica migración). Pendiente: mostrar el conteo en el frontend. Etapa G1 HECHA (`737043e`, CAMBIOS §101): **total real cuando `TNI_DCTO`=0** (la Contaplus real lo trae en 0) → `FacturaDto.TotalNeto` cae a `TSI+IVA` (getter con backing field, recalcula datos existentes y nuevos sin re-extraer) + consulta on-demand con `CASE`. Arregla de raíz: $0.00 en factura/consultas/evidencia, scoring plano (todo 50/Medio) y reglas con umbral de monto que nunca disparaban. Gate verde 337 (+3 `FacturaDtoTests`). El lado central/detector queda vivo al reconstruir la API; la pantalla de factura/consulta requiere que el agente de cada estación tome el build (cambio en el agente). **Pendiente G2** (recalibrar despachos rápidos: keyear por placa + agregación + ámbito Ambos), **G3** (nombre de despachador `DD…` + encoding Ñ "MUÃ?OZ"). — Antes: **NUEVA ronda ERP/UX-2 (pedido de auditoría) — A–F COMPLETA. Etapa F HECHA (`c426d58`, CAMBIOS §100): **cuadre de liquidaciones** — para las liquidaciones de un periodo, ver **qué facturas componen cada una** (cruce `LIQU.NUM_TURN ↔ DCTO.NUM_TURN`, total real `TSI_DCTO+IVA_DCTO` porque `TNI_DCTO` viene en 0 en el backup); agente `ConsultarLiquidacionesAsync` + Worker enruta `LIQUIDACIONES`; página `/consultas/liquidaciones` agrupa las facturas bajo cada liquidación (faltante/sobrante + total) + imprimir + enlace desde Consultas. QA en vivo con backup REAL: EST-001 → 4 liquidaciones con sus facturas y totales reales (1000 celdas con valores, ya no $0.00); gate verde 334. Etapa E HECHA (`62eb3bf`, CAMBIOS §99): **dashboard rediseñado como centro de mando ACCIONABLE** — feed "Atención inmediata" (alertas recientes clicables = triage), KPIs con **drill-down** (→ bandeja filtrada), tendencia con **periodo 7/14/30/90 + delta**, **drill-down en gráficos** (tipo/nivel/estación), filas de empleados clicables, métricas con **semántica de color**; `AlertasPage` lee filtros `?nivel/?estado/?tipo/?estacionId` de la URL. Investigué referencias de dashboards de fraude/SOC. QA Chrome OK (feed con 7 alertas, 5 KPIs-enlace, sin errores). Falta solo **F** (reporte liquidación→facturas). Etapa D HECHA (`ab17578`, CAMBIOS §98): **auditoría del agente** (reactivo: heartbeat 1 s/config en vivo/store-and-forward; escalable: watermark/fuentes configurables/aislamiento; dinámico: auto-extrae+auto-relaciona+reporta esquema) + **consultar CUALQUIER tabla auto-estructurada** — agente `ConsultarTablaGenericaAsync` (`SELECT FIRST n *` con lista blanca anti-inyección) + página **"Explorar tabla"** (`/consultas/tabla`) con columnas dinámicas. QA en vivo: VEND→6 columnas+11 filas; tabla inexistente rechazada sin correr SQL. Etapa C HECHA (`6e8da8d`, CAMBIOS §97): factura enriquecida tipo ERP — nombre de cliente (CLIE) + despachador (VEND) + chofer/autorización(SRI)/guía/observaciones/subtotal, tipo de documento y forma de pago legibles, presentación tipo factura + nombre de cliente en Consultas. **Probada con SIMULACIÓN REAL en Firebird:** factura `001-001-009900070` a cliente real `ZZ0000176` (LOOR CEVALLOS MARIO ANDRES) + despachador `010` (CARLA VALAREZO) + línea DESP → el AGENTE la extrajo por marca de agua (Datos recibidos #3393/#3394, Procesada) y la factura la muestra enriquecida. Fix de rendimiento: se quitó `NOM_CLIE` del WHERE (forzaba join por fila → timeout). Método (registrado en PROMPT-REINICIO §3): insertar en Firebird con datos reales, leyendo el Firebird antes. Pendiente: D/E/F. — Etapas A+B HECHAS y verificadas en vivo en Chrome (commit `3050c94`, CAMBIOS §96):** A) Consultas con deep-link autobuscable (`?est&codigo&tipo&desde&hasta`), inputs de fecha visibles en tema oscuro (`color-scheme:dark`), botón **Imprimir** de lo filtrado y celdas cliente/RUC/placa/despachador que abren la **consulta relacionada en ventana nueva**; B) los chips de evidencia de la alerta abren la consulta **RELACIONADA en vivo en ventana nueva** (ya NO `/alertas?buscar`), el nº de factura sigue abriendo la **factura completa**. QA: alerta #80 → 13 enlaces de factura, 0 a `/alertas?buscar`, 7 RUC relacionables; deep-link por RUC → 6 docs reales. **Pendiente de la ronda: C** factura con más datos + presentación, **D** auditoría del agente + consultar tabla X auto-estructurada, **E** rediseño del dashboard, **F** reporte liquidación→facturas. — Ronda anterior: **Ronda ERP/UX COMPLETA — las 5 etapas HECHAS y verificadas en vivo:** (1) refresco configurable + agente 1 s `626c3e0`; (2) evidencia identificable automática `1114d65`; (3+4) **consulta en vivo a Firebird** `8d0e651`+`7166e40`+`699dbb1`+`5f149a9` (cola on-demand agente+central + pestaña Consultas + factura en ventana nueva; QA en vivo §92: RUC `1301790737` → 21 FV reales; se cazó/arregló un `string right truncation` de Firebird con `CAST(col AS VARCHAR(60))` antes de `CONTAINING` + normalización de claves); (5) **detalle de alerta → factura completa** `e0a1f4f` (§93): el nº de factura de la evidencia abre la `FacturaPage` en ventana nueva (`AlertaResponse.EstacionCodigo` nuevo; QA en vivo: alerta #80 → factura `001-001-009900060` total $500). **Cierre de pendientes (§94, `25334b6`/`ac371aa`/`f239a3a`):** `Fuente` extendida a las 4 reglas de factura restantes (FueraHorario, FechaFueraDeRango, ReversionTardia, DespachosRapidos → heredan RUC/nº doc/placa/cliente/turno/forma de pago/monto); **«Despacho no facturado» DESACTIVADA por defecto (#136)** porque FAC_DESP es forma de pago, no «facturado» (QA en vivo: toggle inactivo); **despachador buscable + Nº de despacho (NDO_DCTO) en la factura** (QA en vivo: 200 docs por «005»; factura real `006102000024646` → «Despacho (origen) 39904»). Y el **detalle de la línea DESP** (producto/galones/precio) ya quedó EN LA FACTURA (`a173b5a`, §95, consulta DESP aislada por `NUM_DESP=CAST(NDO_DCTO)`; QA en vivo: factura `006102000024646` → 1.01 gal @ $2.47 = $2.50). **El round ERP/UX no tiene pendientes.** Antes: **Mejora #3 cuadre de turno HECHA** `43fe272` + gate verde + QA E2E en vivo OK: alerta #80 "turno 990001 sin liquidación". Antes, **QA en vivo en Chrome OK** de #1 regla placa, #2 buscador y #4 dashboard por estación; se cazó y arregló en vivo un **500 del buscador** —`MetadataJson` es jsonb, `c011374`—. Lectura completa de las 6 carpetas de `ejecutables`. Antes: #1 `99d3330`, #2 `e5d7bdb`, #4 `594f186`, verificación #3 `cd4008f`).

## 🧾 Backlog de auditoría (entrevista Juan Valdez) — ver docs/ANALISIS-CONTAPLUS-Y-ENTREVISTA-AUDITORIA.md
- [x] 🔴 **Misma placa facturada N+ veces al día** (reutilización de placa; caso real 14×/día, riesgo SRI). **HECHO `99d3330`**: `PlacaReutilizadaRule` (InvoiceAnomaly), programada **diaria** (Calendario 23:55, ventana del día, sin duplicados), umbral configurable `PlacaReutilizadaDiaUmbral` (default 5; auditora sugiere 2), excluye placa genérica. +7 pruebas → Detectors 189. **FIX `4aa24d3` (§104.3):** la ventana de la Pasada B ahora es por **`CreatedAt`** (llegada), no `FechaOriginal` → la regla ya ve datos cargados en bloque/tardíos (antes 79 pares placa/día con +5 facturas daban 1 sola alerta). *Verificable tras un re-sync.*
