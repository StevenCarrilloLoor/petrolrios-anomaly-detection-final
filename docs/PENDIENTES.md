...
Última actualización: 27 de junio de 2026 (**QA en vivo en Chrome OK** de #1 regla placa, #2 buscador y #4 dashboard por estación; se cazó y arregló en vivo un **500 del buscador** —`MetadataJson` es jsonb, `c011374`—. Lectura completa de las 6 carpetas de `ejecutables`. Antes: #1 `99d3330`, #2 `e5d7bdb`, #4 `594f186`, verificación #3 `cd4008f`).
...
- [~] 🔴 **Factura no incluida en ninguna liquidación** (cuadre de turno, mejora #3). **✅ ENLACE VERIFICADO contra la Firebird real (CONTACONSTANZA, vía Docker `petrolrios-firebird`):** `LIQU`=149 filas, **149/149 con `NUM_TURN`** (puebla 100%); `DCTO`=40 046 (FV 39 958 / DV 88); de **39 041 FV con turno, solo 25 sin liquidación (0,06%)**, y esas 25 son del **turno sintético 990001** (inserts de prueba) — el histórico real (2025) está 100% liquidado. Conclusión: `DCTO.NUM_TURN ↔ LIQU.NUM_TURN` es fiable; la regla es construible (NO es un caso FAC_DESP). **Pasos que faltan:** (1) **cambiar el agente** (`StationAgent` lee `LIQU` y la envía como tipo "Liquidacion") + **republicar** 4 plataformas; (2) `LiquidacionDto` + `DetectionContext.Liquidaciones` + `ConstruirContexto`; (3) regla. **⚠️ Reto de diseño (ventana):** el Pass B acota staging por `FechaOriginal`; una factura y la `LIQU` de su turno pueden caer en ventanas distintas → para una verificación de *ausencia* correcta conviene (a) **persistir las liquidaciones** en una tabla del central (no solo staging transitorio) y que la regla consulte todas, o (b) que el agente mande un **flag "turno liquidado"** por factura. Recomendado (a). La regla debe keyear sobre **turnos CERRADOS** (con cierre en `TURN`) sin `LIQU`, no sobre turnos abiertos aún sin liquidar. (4) tests + seed (programada) + gate + Chrome.
...
- [~] 🟠 UX de alertas (mejora #2, **HECHO `e5d7bdb` + fix `c011374`, QA en vivo OK**): ✅ buscador (placa/nº factura/cliente/código vía descripción), ✅ nº factura y placa/cliente como **enlaces** a la bandeja filtrada, ✅ **abrir en ventana nueva**, ✅ **copiar**. ⚠️ **Limitación:** búsqueda por **RUC** y **nombre de despachador** no cubierta (RUC vive solo en evidencia `jsonb`; el nombre se resuelve aparte del código) — mejora futura: cruzar catálogo de empleados y/o exponer RUC en columna de texto. ⏳ Pendiente aún: "ver todas las **facturas** del cliente" (reporte sobre datos de origen) y despachador→despachos+imprimir.
